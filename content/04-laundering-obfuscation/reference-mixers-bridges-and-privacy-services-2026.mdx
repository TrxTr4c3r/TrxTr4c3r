...
6. **Shrink it.** In the order that pays best:
   - **Relayer.** Read `relayer` from the withdrawal. Then fingerprint it by fee: on the 10 ETH pool I observed `0x0A5B2bF3cCfB44C1D22F07Eed9553eCba752D4aD` charging exactly 0.04015 ETH on both its withdrawals while `0x4750BCfcC340AA4B31be7e71fa072716d28c29C5`, `0xAAAAA27EA9ffab81F4735e6B766af985d0c88C61`, `0x000000Cd6521Ed1a65FAe0678eA15aF4EEAD74fe` and `0xEE4C45Cc5eAa535CB8a8ffdc92f2839A601fe226` all charged exactly 0.04465 ETH. **A distinct fee is a relayer fingerprint**, so an unlabelled withdrawal can be attributed to a relayer by fee alone, and relayer choice clusters users.
...
Blockscout still tags all four ETH pools `OFAC Sanctions Lists`, `SANCTIONED` and `BLOCKED BY USDC` or `BLOCKED BY USDT`. Explorer labels lag reality in both directions. Check the live SDN list yourself before you assert a sanctions nexus in a report, and never source a sanctions claim from an explorer tag.

...
- Three service types (who even is there to ask):
  ...
  - Bridge intervention, when a freeze rather than a mapping is the goal: with no compliance desk to serve, the routes are the development and security team directly, the DAO or governance process where the bridge has one, and the validator or guardian set in parallel. Prepare a technical package specific to that bridge's architecture. Treat any published list of which bridges can freeze as perishable, because several named in older guidance no longer operate.

...
| THORNode API | Current and historical Asgard inbound vault addresses | free | thornode.ninerealms.com | `/thorchain/inbound_addresses`. Vaults churn circa every 3 days, so NEVER cache a vault address: resolve it for the relevant date |
| Midgard API | THORChain swap records linking inbound to outbound | free | midgard.ninerealms.com | Often the only reliable link: the legs are separate txs on different chains joined only by THORChain state. Read the memo, which frequently carries the destination address in clear text |
